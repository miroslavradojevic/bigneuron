...
static unsigned char imgTh = 1;
static float    gcsstd2rad = 2.0; // scaling between gaussian cross section standard deviation and the neurite radius
static int      nonMaxSuppBeltScale = 1;
...
//        PARA.gcsSig = gcsSig;
        ...
        items.push_back("Foreground percentile (1-100)");
...
//        CommonDialog dialog(items);
        ...
        dialog.get_num("Foreground percentile (1-100)", PARA.perc);
...
//        dialog.get_num("Gaussian Cross Section sigma (1-10) [pix]", PARA.gcsSig);
        dialog.get_num("# iterations (2-50)", PARA.Ni);
        dialog.get_num("# states (1-20)", PARA.Ns);
        ...
        if(PARA.perc < 1        || PARA.perc > 100)      {v3d_msg(QObject::tr("Percentile is out of range")); return;}
...
void print_help(){
    ...
    printf("perc                Percentile (1-100) (for extracting foreground).\n");
...
//        PARA.gcsSig     = (paras.size() >= k+1)   ? QString(paras[k]).toFloat() : gcsSig;    k++;
        ...
        if(PARA.perc < 1        || PARA.perc > 100)      {v3d_msg(QObject::tr("Percentile is out of range")); return false;}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    cout << "\nforeground score..."<<flush;

    unsigned char * scr = new unsigned char [size]; // foreground score
    for (long var = 0; var < size; ++var) scr[var] = 0;

    int range2 = PARA.scal;
    ...
    for(long i = 0; i < size; i++) {
        ...
        if (x>=range2 && x<N-range2 && y>=range2 && y<M-range2  && data1d[i]>imgTh ) {
            ...
            m05 = quantile(nbhood, (2*range2+1)*(2*range2+1), 5,  100); // TODO both in one go
            m95 = quantile(nbhood, (2*range2+1)*(2*range2+1), 95, 100);
    ...
    for (long i = 0; i < size; ++i) {
        ...
        if (x>=rang_xy && x<N-rang_xy && y>=rang_xy && y<M-rang_xy && z>=rang_z && z<P-rang_z) {
    ...
    if (PARA.saveMidres) {
        // save int* tagmap as set of .txt images (uint8 is not enough and saving float image regular way did not work)
        cout << "exporting tagmap (foreground-background) z -- ";
        for (int zz = 0; zz < P; ++zz) {
            cout << zz << ", "<<flush;
            QString of = PARA.inimg_file + "_fgbg_"+QString("%1").arg(zz, 4, 10, QChar('0'))+".txt";
            ofstream myfile;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    int rxout, ryout, rzout;  // gcsstd_max * scale
    int rxin,  ryin,  rzin;   // gcsstd * scale
    ...
    for (long i = 0; i < cnt_fg; ++i) { // go through all foreground locations (initially tagged as -1)
        ...
        if (tagmap[z*N*M+y*N+x]==-1) { // foreground location that can be a candidate for a node center
            ...
            if (i2calcs[i][0]>PARA.znccTh) { // candidate for guidepoint, check local neighbourhood
                            ...
                            if (!(xn==x && yn==y && zn==z)) { // skip the midpoint
                                if (xn>=0 && xn<N && yn>=0 && yn<M && zn>=0 && zn<P) { // is in image
                                    if (tagmap[zn*N*M+yn*N+xn]==-1) { // not in background, not in other region and would not overlap
                                        if (pow(xn-x,2)/pow(rx,2)+pow(yn-y,2)/pow(ry,2)+pow(zn-z,2)/pow(rz,2)<=1) { // in the sphere
                                            ...
                                            if (i2calcs[in][0] == -FLT_MAX) { // calculate value if not calculated
                                                ...
                                                for (int dix = 0; dix < PARA.Ndir; ++dix) {
                                                    ...
                                                    curr_zncc = btrcr.zncc(xn, yn, zn, vx_corr[dix], vy_corr[dix], vz_corr[dix],
                                                                data1d, N, M, P,
                                                                curr_gcsstd,
                                                                false);
                ...
                rxin  =  ryin  =        round(gcsstd2rad * i2calcs[lmax_i][1]);
                rzin  =                 round(gcsstd2rad * i2calcs[lmax_i][1]/zDist);

                rxout =  ryout =        rxin + nonMaxSuppBeltScale * round(gcsstd2rad * btrcr.gcsstd_max);
                rzout =                 rzin + nonMaxSuppBeltScale * round(gcsstd2rad * btrcr.gcsstd_max / zDist);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (PARA.saveMidres) {
        // save int* tagmap as set of .txt images (uint8 is not enough and saving float image regular way did not work)
        cout << "exporting tagmap";
        for (int zz = 0; zz < P; ++zz) {
            cout << "." << flush;
            QString of = PARA.inimg_file + "_fgbggpnt_"+QString("%1").arg(zz, 4, 10, QChar('0'))+".txt";
            ofstream myfile;
            myfile.open (of.toStdString().c_str());
            for (int yy = 0; yy < M; ++yy) {
                for (int xx = 0; xx < N; ++xx) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int ni = 0; ni < indices.size(); ++ni) {
        ...
        // print process
        if (ni!=0 && ni%(indices.size()/10)==0) cout << "\t--- " <<ni/(indices.size()/10)*10 << "%\t" << endl;
...
//            for (int var = 0; var < outcome_tags.size(); ++var) {
//                cout << outcome_tags[var] << " " << flush;
//            }
//            cout << endl;
...
//                        cout << ti <<" "<< nodetag << "<->" << nodetag-1 <<" " << flush;
                    ...
                    float rxy_sph = gcsstd2rad * nodelist.at(nodetag).r;
                    float rz_sph  = (gcsstd2rad * nodelist.at(nodetag).r)/zDist;
    ...
    cout << "done. nodelist = " << nodelist.size() << " nodes" << endl;
    ...
    for (int ni = 1; ni < nodelist.size(); ++ni) {
        ...
        for (int i = 0; i < nodelist[ni].nbr.size(); ++i) {
            cout << nodelist[ni].nbr[i] << " " << flush;
        }
    ...
    if (PARA.saveMidres) {
        ...
        for (int ii = 1; ii < nodelist.size(); ++ii) {
            ...
            n0.r = gcsstd2rad * nodelist.at(ii).r;
            n0.parent = -1;
    ...
    if (false) {
        // export final tagmap with nodelist tags as they were added
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    int prune_len = 2; // those that reach endpoint after 1 step are removed from the reconstrction to clean the tree
    ...
    while ( (seed_idx = get_undiscovered(indices, discovered))!=-1) { // seeds are undiscovered nodes
        ...
        for (int i = 0; i < seed.size(); i++) {
            ...
            nseed.r = gcsstd2rad * nodelist[curr].r;
            nseed.parent = -1;
        ...
        while (boob.hasItems()) {
            ...
            if (nadd) {
                ...
                n0.r = gcsstd2rad * nodelist[curr].r;
            ...
            while (((count_undiscovered = count_vals(discovered[curr], false)) == 1) && nadd) {
                ...
                if (nadd) {
                    ...
                    n1.r = gcsstd2rad * nodelist[curr].r;
...
//            if (count_new_branches!=count_undiscovered) cout << "WRONG"<<endl;
            ...
            if ((count_undiscovered==0 || !nadd) && bchlen<=prune_len) {
                ...
                for (int ii = 1; ii <= bchlen; ++ii) {
                    ...
                    cout<<" new size = "<<ntcurr.listNeuron.size()<<endl;
