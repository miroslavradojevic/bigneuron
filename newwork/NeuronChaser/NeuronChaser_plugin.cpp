...
//    cout << "init tagmap" << endl;
    ...
    // threshold
    unsigned char data1d_min = data1d[0];
    unsigned char data1d_max = data1d[0];

    for (long i = 0; i < size; ++i) {
        if (data1d[i]<data1d_min) data1d_min=data1d[i];
        if (data1d[i]>data1d_max) data1d_max=data1d[i];
    }

    int cntvals = 0;

    for (long i = 0; i < size; ++i) {
        if (data1d[i]<data1d_max) { // data1d[i]>data1d_min &&
            cntvals++;
        }
    }

    unsigned char * data1d_temp = new unsigned char[cntvals];

    cntvals = 0;
    for (long i = 0; i < size; ++i) {
        if (data1d[i]<data1d_max) { // data1d[i]>data1d_min &&
            data1d_temp[cntvals] = data1d[i];
            cntvals++;
        }
    }

    unsigned char th1 = intermodes_th(data1d_temp, cntvals);
    delete data1d_temp; data1d_temp = 0;

    // assign those tagmap that are in the foreground (thresholded) with -1
    // and also their neighbourhood
    for (long i = 0; i < size; ++i) {
        if (data1d[i]>=th1 && tagmap[i]==0)
            tagmap[i] = -1;
    }
...
//    delete scr1; scr1 = 0;
    ...
    if (PARA.saveMidres) {
        ...
        QString of = PARA.inimg_file + "_interm.tif";
